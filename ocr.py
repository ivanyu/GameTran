...
def _get_ocr(ocr_image_base64: str, google_api_key: str) -> OCRResult:
    if os.environ.get("GT_DEV") == "true":
        ...
        if not cache_path.exists():
            response_json = _request_ocr(ocr_image_base64, google_api_key)
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            with open(cache_path, "w", encoding="utf-8") as f:
                json.dump(response_json, f, ensure_ascii=False, indent=4)
            return _make_ocr_result(response_json)
        else:
            with open(cache_path, "r", encoding="utf-8") as f:
                return _make_ocr_result(json.load(f))
    else:
        response_json = _request_ocr(ocr_image_base64, google_api_key)
        return _make_ocr_result(response_json)


def _request_ocr(ocr_image_base64: str, google_api_key: str) -> dict[str, Any]:
    request = {
        "requests": [
            {
...
def _make_ocr_result(response_json: dict[str, Any]) -> OCRResult:
    page = response_json["responses"][0]["fullTextAnnotation"]["pages"][0]
    ...
    for block in page["blocks"]:
        ...
        for paragraph in block["paragraphs"]:
            for word_json in paragraph["words"]:
                ...
                word_id += 1
                words.append(word)
    return OCRResult(detectedLanguage=detected_language, words=words)
